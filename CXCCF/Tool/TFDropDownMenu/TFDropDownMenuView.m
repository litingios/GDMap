...
- (UIView *)bottomLineView {
    if (!_bottomLineView) {
        _bottomLineView = [[UIView alloc] initWithFrame:CGRectMake(0, self.bounds.size.height-(1.0/SCREEN_SCALE), kScreenWidth, (1.0/SCREEN_SCALE))];
...
- (UITableView *)leftTableView {
    if (!_leftTableView) {
        _leftTableView = [[UITableView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y + self.bounds.size.height, kScreenWidth, 0)];
...
- (UITableView *)rightTableView {
    if (!_rightTableView) {
        _rightTableView = [[UITableView alloc] initWithFrame:CGRectMake(self.frame.origin.x + self.bounds.size.width, self.frame.origin.y + self.bounds.size.height, self.bounds.size.width * (1 - _ratioLeftToScreen), 0)];
...
- (UICollectionView *)leftCollectionView {
    if (!_leftCollectionView) {
        ...
        _leftCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y + self.bounds.size.height, self.bounds.size.width, 0) collectionViewLayout:collectionViewLayout];
...
- (UICollectionView *)rightCollectionView {
    if (!_rightCollectionView) {
        UICollectionViewFlowLayout *collectionViewLayout = [[UICollectionViewFlowLayout alloc] init];
        _rightCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(self.frame.origin.x + self.bounds.size.width, self.frame.origin.y + self.bounds.size.height, self.bounds.size.width * (1 - _ratioLeftToScreen), 0) collectionViewLayout:collectionViewLayout];
...
- (UIScrollView *)customScrollView {
    if (!_customScrollView) {
        _customScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(self.frame.origin.x + self.bounds.size.width, self.frame.origin.y + self.bounds.size.height, self.bounds.size.width * (1 - _ratioLeftToScreen), 0)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == _leftTableView) {
        // 一级列表
        cell.textLabel.text = [self titleForColumn:_currentSelectColumn section:indexPath.row+1];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_delegate) {
        ...
        if (tableView == _leftTableView) {
            ...
            _currentSelectSections[_currentSelectColumn] = [NSNumber numberWithInteger:indexPath.row+1];
            ...
            if (haveItems) {
                ...
                // 收回列表
                titleLayer.string = [self titleForColumn:_currentSelectColumn section:indexPath.row+1];
    ...
    if (haveItems) {
        switch (style) {
            case TFDropDownMenuStyleTableView: {
                ...
                self.leftTableView.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y + self.bounds.size.height, kScreenWidth, 0);
                self.rightTableView.frame = CGRectMake(self.frame.origin.x + self.bounds.size.width, self.frame.origin.y + self.bounds.size.height, self.bounds.size.width * (1 - _ratioLeftToScreen), 0);
                [self.superview addSubview:self.leftTableView];
                [self.superview addSubview:self.rightTableView];
                [UIView animateWithDuration:_kAnimationDuration animations:^{
                    self.leftTableView.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y + self.bounds.size.height, kScreenWidth, heightForTableView);
