...
-(instancetype)init{
    self = [super init];
    if (self) {
        self.backgroundColor = rgb(29, 29, 29);
...
-(void)buildTags{
    ...
    for (int i = 0; i<_dataA.count; i++) {
        CXTipsModel *model = _dataA[i];
        UIButton *button;
        button =[UIButton LXButtonNoFrameWithTitle:model.categoryName titleFont:self.tagFont Image:nil backgroundImage:nil backgroundColor:rgb(51, 51, 51) titleColor:[UIColor whiteColor]];
        
        button.layer.cornerRadius = 3;
        button.layer.masksToBounds = YES;
        if (model.isSelect) {
            [button setBackgroundColor:rgb(32, 136, 203)];
        }
        __weakSelf;
//
        
        [button addClickBlock:^(UIButton *button) {
            for (int j = 0 ; j< weakSelf.dataA.count ; j ++) {
                CXTipsModel *model1 = weakSelf.dataA[j];
                if (i == j) {
                    model1.isSelect = YES;
                }else{
                    model1.isSelect = NO;
            ...
            weakSelf.tagClick(weakSelf.dataA);
        }];
...
-(void)layoutItems{
    ...
    for (NSUInteger i = 0; i< count; i ++) {
        ...
        CXTipsModel *model = self.dataA[i];
        NSString *tagTitle = model.categoryName;
